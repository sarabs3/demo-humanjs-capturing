const data = {
    "<-60": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": { 
            "pitch_bin": "25",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "<-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-60": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-55": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-55",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-50": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-45": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-45",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-40": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-40",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-40",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-35": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-35",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-35",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-30": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-30",
            "count": 5,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-25": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-25",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-25",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-20": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-20",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-15": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-15",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-15",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-15",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-10": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-10",
            "count": 6,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-10",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-10",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "-5": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "-5",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "-5",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "-5",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "-5",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "-5",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "-5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "5": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "5",
            "count": 23,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "5",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "5",
            "count": 5,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "5",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "5",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "5",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "10": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "10",
            "count": 82,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "10",
            "count": 62,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "10",
            "count": 7,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "10",
            "count": 6,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "10",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "15": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "15",
            "count": 7,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "15",
            "count": 11,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "15",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "15",
            "count": 56,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "15",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "20": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "20",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "20",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "20",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "20",
            "count": 5,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "20",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "20",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "25": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "25",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "25",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "25",
            "count": 10,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "25",
            "count": 39,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "25",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "30": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "30",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "30",
            "count": 41,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "30",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "35": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "35",
            "count": 17,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "35",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "35",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "40": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "40",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "40",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "40",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "45": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "45",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "45",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "45",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "50": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "50",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "50",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "50",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "50",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "50",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "55": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "55",
            "count": 2,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "55",
            "count": 22,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "55",
            "count": 1,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "55",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    "60": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": "60",
            "count": 3,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": "60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    },
    ">60": {
        ">40": {
            "pitch_bin": ">40",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "40": {
            "pitch_bin": "40",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "35": {
            "pitch_bin": "35",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "30": {
            "pitch_bin": "30",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "25": {
            "pitch_bin": "25",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "20": {
            "pitch_bin": "20",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "15": {
            "pitch_bin": "15",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "10": {
            "pitch_bin": "10",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "5": {
            "pitch_bin": "5",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-5": {
            "pitch_bin": "-5",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-10": {
            "pitch_bin": "-10",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-15": {
            "pitch_bin": "-15",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-20": {
            "pitch_bin": "-20",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-25": {
            "pitch_bin": "-25",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-30": {
            "pitch_bin": "-30",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-35": {
            "pitch_bin": "-35",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "-40": {
            "pitch_bin": "-40",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        },
        "<-40": {
            "pitch_bin": "<-40",
            "yaw_bin": ">60",
            "count": 0,
            "expressions": [
                "smiling",
                "mouth open",
                "eyes open",
                "teeth showing"
            ]
        }
    }
}
const yawPitchData = Object.keys(data) // .filter(k => !k)

let validYawPitchData = [];
yawPitchData.forEach((k) => {
    const item = data[k];
    const child = Object.keys(item);
    child.forEach(l => {
        const childItem = item[l];
        if (childItem.count > 0) {
            validYawPitchData.push(childItem)
        }
    });
});


let topLeft = { values: [] };
const bottomRight = { values: [] };
const topRight = { values: [] };
const bottomleft = { values: [] };
const remaining = [];
validYawPitchData.forEach((f) => {
    if(parseInt(f.pitch_bin) < 0 && parseInt(f.yaw_bin) < 0) {
        topLeft.values.push({ yaw: parseInt(f.yaw_bin), pitch: parseInt(f.pitch_bin), valid: false });
    }
    else if (parseInt(f.pitch_bin) > 0 && parseInt(f.yaw_bin) > 0) {
        bottomRight.values.push({ yaw: parseInt(f.yaw_bin), pitch: parseInt(f.pitch_bin), valid: false });
    }
    else if(parseInt(f.pitch_bin) < 0 && parseInt(f.yaw_bin) > 0) {
        topRight.values.push({ yaw: parseInt(f.yaw_bin), pitch: parseInt(f.pitch_bin), valid: false });
    }
    else if(parseInt(f.pitch_bin) > 0 && parseInt(f.yaw_bin) < 0) {
        bottomleft.values.push({ yaw: parseInt(f.yaw_bin), pitch: parseInt(f.pitch_bin), valid: false });
    } else {
        remaining.push({ yaw: parseInt(f.yaw_bin), pitch: parseInt(f.pitch_bin), valid: false });
    }
});
topLeft.highestPitch = topLeft.values.map(p => p.pitch).sort((a,b) => a-b)[0]
topLeft.highestYaw = topLeft.values.map(p => p.yaw).sort((a,b) => a-b)[0]

bottomRight.highestPitch = bottomRight.values.map(p => p.pitch).sort((a,b) => a-b)[0]
bottomRight.highestYaw = bottomRight.values.map(p => p.yaw).sort((a,b) => a-b)[0]

topRight.highestPitch = topRight.values.map(p => p.pitch).sort((a,b) => a-b)[0]
topRight.highestYaw = topRight.values.map(p => p.yaw).sort((a,b) => a-b)[0]
bottomleft.highestPitch = bottomleft.values.map(p => p.pitch).sort((a,b) => a-b)[0]
bottomleft.highestYaw = bottomleft.values.map(p => p.yaw).sort((a,b) => a-b)[0]


const all = {
    total: validYawPitchData,
    topLeft,
    bottomRight,
    topRight,
    bottomleft,
    remaining,
};
console.log(topLeft.values);
// export default all;
